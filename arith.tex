...
\begin{document}
	...
	Here, we can see that the function
		...
		\end{align*}
	is also quite important. In fact, it is the partial sum of Euler's \index{Zeta function}\textit{zeta function}.
	...
	Here, $\sim$ is the asymptotic equivalence we have already defined. Even though Euler is the main architect behind the development of $\zeta$, there are compelling reasons why it is called Riemann's zeta function. \textcite{riemann_1859} (which is the only paper on number theory by Riemann) first considered $\zeta$ for complex $s$ instead of real $s$ only. This also gave new insight of primes and Riemann conjectured that the non-trivial zeros of $\zeta$ lie in the \index{Critical line}\textit{critical line}
		...
		\end{align*}
	which is now known as the \index{Riemann hypothesis}\textit{Riemann hypothesis}. Riemann hypothesis is considered to be one of the greatest unsolved mysteries in mathematics. Let us establish a result similar to \nameref{thm:dirtau} for partial sums of $\zeta$. Setting $f(n):=n^{-s}$ and $a_{n}=1$ in \nameref{thm:abel}, $A(x)=\lfloor{x}\rfloor=x+O(1)$ and
		\begin{align*}
	...
	Similar to $\gamma$, we can take $x\to\infty$ and get the following result.
		\begin{theorem}
			...
			where $C$ is a constant similar to Euler-Mascheroni constant dependent on $s$ and
				...
				\end{align*}
			Furthermore, if $0<s<1$, then $C=\zeta(s)$ since $x^{-s}\to 0$.
		\end{theorem}
