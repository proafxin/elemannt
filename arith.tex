...
\begin{document}
	...
	We can use this to inductively establish an asymptotic for $A_{k}(x)$ if we can establish the asymptotic of $A_{2}(x)$. We will get to that in a moment. First, let us take care of the summation within the big O bracket. We have the trivial inequality that
		\begin{align*}
			\sum_{n\leq x}n^{k}
				...
				& = x^{k+1}+O(x^{k})
		\end{align*}
	We have that $A_{k}(x)=x\left(x^{k}+O(x^{k-1})\right)+O(x^{k+1})=O(x^{k+1})$. Although this is a weak estimate, it shows a method of estimation. On this note, an interested reader can try and prove that
		\begin{align*}
			(n+1)^{k+1}-1
	...
	Using a similar approach, we will now estimate $B$.
\end{document}