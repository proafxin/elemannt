...
\begin{document}
	...
	Here, $\sim$ is the asymptotic equivalence we have already defined. Even though Euler is the main architect behind the development of $\zeta$, there are compelling reasons why it is called Riemann's zeta function. \textcite{riemann_1859} (which is the only paper on number theory by Riemann) first considered $\zeta$ for complex $s$ instead of real $s$ only. This also gave new insight of primes and Riemann conjectured that the non-trivial zeros of $\zeta$ lie in the \index{Critical line}\textit{critical line}
		...
		\end{align*}
	which is now known as the \index{Riemann hypothesis}\textit{Riemann hypothesis}. Riemann hypothesis is considered to be one of the greatest unsolved mysteries in mathematics. This topic is right now out of the scope of this book, so let us back to establishing a result similar to \nameref{thm:dirtau} for partial sums of $\zeta$. Setting $f(n):=n^{-s}$ and $a_{n}=1$ in \nameref{thm:abel}, $A(x)=\lfloor{x}\rfloor=x+O(1)$ and
		\begin{align*}
	...
	We leave the case when $k$ is a negative integer as an exercise. For our next function, let us consider a generalization of the Euler's totient function $\varphi(n)$.
		...
		\end{align*}
	For a positive integer $n$, $\varphi(n)=\varphi(n,n)$ and \index{Jordan function}\textit{Jordan function}.
