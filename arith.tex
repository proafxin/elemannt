...
\begin{document}
	...
	In this chapter, we will discuss some generalized arithmetic functions and their asymptotic behavior. We will skip discussing the basic definitions since they are common in most introductory number theory texts. See \gls{erdos}, \gls{mobius}
		...
		\end{definition}
	Note that the number of divisor function $\tau(n)$ is the summatory function of the unit function $u(n)=1$ where $\mathbb{D}$ is the set of divisors of $n$. The sum of divisor function $\sigma(n)$ is the summatory function of the invariant function $f(n)=n$ where $\mathbb{D}$ is the set of divisors of $n$. Another interesting summatory function we will see is functions of the form
		\begin{align*}
			\sum_{n\leq x}f(n)
		\end{align*}
	...
	We have that $S_{k}(x)=x\left(x^{k}+O(x^{k-1})\right)+O(x^{k+1})=O(x^{k+1})$. Although weak, we get an estimate this way. On this note, an interested reader can try and prove that
		...
		\end{align*}
	This is known as the \index{Pascal identity}\textit{Pascal identity} (see \textcite{pascal_1964}, for an English translation, see \textcite{knoebel_laubenbacher_lodder_pengelley_2007}). The reader may also be interested in \textcite{kieren_macmillan_jonathan_sondow_2011}. \textcite[Chapter II, Theorem $1$]{lehmer_1900} proves that
		\begin{align*}
			\sum_{n\leq x}^{k}
				& = \dfrac{x^{k+1}}{k+1}+\Delta
		\end{align*}
	where $|\Delta|\leq x^{n}$. Let us try to estimate $T$ in a similar fashion. First, see that
		\begin{align*}
			\tau_{k}(n)
				& = \sum_{d_{1}\cdots d_{k}=n}1\\
