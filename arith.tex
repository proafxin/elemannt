	...
	For example, the prime counting function is
		\begin{align*}
			\pi(x)
				& = \sum_{n\leq x}C(n)
		\end{align*}
	where $C(n)$ is the characteristic function of, that is, $C(n)=1$ if $n$ is a prime otherwise $C(n)=0$. One of the biggest questions we will try to answer is how $\lim_{x\to\infty}\pi(x)$ behaves.
