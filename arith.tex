...
\begin{document}
	...
	Consider the \textbf{harmonic sum}
		\begin{align*}
			H(n)
				& = 1+\dfrac{1}{2}+\ldots+\dfrac{1}{n}
		\end{align*}
	It does not seem easy to calculate $H(n)$ accurately, however, we can make a decent attempt to estimate $H(n)$. The tool that is best suited for carrying out such an estimation is the \textbf{Abel partial summation formula}, which today is a cornerstone of analytic number theory.
		...
		\end{proof}
	It is not straightforward to realize how such a formula can be as influential as we are making it out to be. Notice that, the formula essentially converts a discreet sum into an integral, which occasionally may be calculable. If the integral is not calculable, we may be able to estimate its value sometimes. We will demonstrate these ideas next.
