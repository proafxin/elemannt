...
\begin{document}
	...
	We have the next theorem about the associativity of $\bullet$ convolution.
		\begin{proposition}[Associativity of Generalized Convolution]\label{pro:genassociativity}
		...
		\begin{proposition}[Inversion of Generalized Convolution]\label{pro:geninverse}
		...
		\begin{proof}
			...
			\begin{align*}
				...
				& = f(\mathbf{x},\mathbf{a})
			\end{align*}
			Since $g=\alpha\bullet f$, we will use \nameref{pro:genassociativity} on $\alpha^{-1}$ and $g$. We have
			\begin{align*}
		...
		\end{proof}
	If we set $\mathbf{a}=(1),k=1$ and $\mathbf{x}=(x)$ for a real number $x$ in \nameref{pro:genassociativity} and \nameref{pro:geninverse}, we have the usual \nameref{thm:genconv} and \nameref{pro:geninverse}.
		\begin{proposition}
	...
	where $g(\mathbf{a})=\gcd(a_{1},\ldots,a_{k})$ for the vector of positive integers $\mathbf{a}=(a_{1},\ldots,a_{k})$. Then we have
		...
		\end{align}
	\eqref{eqn:Sx1} follows from \nameref{pro:genassociativity} and \eqref{eqn:inversejordan}.
		\begin{align*}
