...
\begin{document}
	...
	Let us call such a set $\mathbb{D}$ \textit{divisor closed}.
		...
		\end{proof}
	While discussing inversion, we should also mention \index{Dirichlet inverse}\textit{Dirichlet inverse}.
		\begin{definition}[Dirichlet Inverse]
			Let $f$ be an arithmetic function such that $f(1)\neq 0$. Then the Dirichlet inverse of $f$ is a function $g$ such that $f\ast g=I$ where $I$ is the \index{Identity function}\textit{identity function}
				\begin{align*}
					I(n)
