...
\begin{document}
	...
	\textcite{tchebycheff_1852} is the first one to make any substantial progress on the matter though we will not discuss his findings right now.
		\begin{definition}[Tschebischeff's theta function]
			Tschebischeff function of the first kind or \index{Tschebischeff's theta function}\textit{Tschebischeff's theta function} is defined as
				\begin{align*}
					\vartheta(x)
						& = \sum_{p\leq x}\log{p}
				\end{align*}
		...
		\begin{definition}[Tschebischeff's psi function]
			Tschebischeff function of the second kind or \index{Tschebischeff's psi function}\textit{Tschebischeff's psi function} is defined as
				\begin{align*}
					\psi(x)
