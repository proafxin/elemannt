...
\begin{document}
	...
	\textcite[pp. 122, Theorem 152]{landau_1969} says the following about this theorem:
		...
		\end{quote}
	Before we prove this, the reader is encouraged to try to prove this. For example, a question is whether \nameref{thm:abel} works here with the following decomposition or not:
		\begin{align*}
	...
	Then taking $x\to\infty$ seems very promising but there is a catch. Figuring out this catch is left to the reader as an exercise.
		\begin{proof}
			...
			Using these, we get
				\begin{align*}
					\varUpsilon(x)
						...
						& = 2\sqrt{x}\sum_{n\leq \sqrt{x}}\dfrac{\chi(n)}{n}+D\sum_{n\leq \sqrt{x}}\dfrac{\chi(n)}{\sqrt{n}}+\dfrac{1}{\sqrt{x}}\bigo{\sum_{n\leq \sqrt{x}}\chi(n)}+C\sum_{n\leq \sqrt{x}}\dfrac{1}{\sqrt{n}}+\dfrac{1}{\sqrt{x}}\bigo{\sum_{n\leq \sqrt{x}}1}-F(\sqrt{x})G(\sqrt{x})\\
						& = 2\sqrt{x}\sum_{n\leq \sqrt{x}}\dfrac{\chi(n)}{n}+D\cdot F(\sqrt{x})+C\cdot G(\sqrt{x})+O(1)-F(\sqrt{x})G(\sqrt{x})
				\end{align*}
			Note that that $ab-ac-bd =(b-c)(a-d)-cd$. Setting $a:=F(\sqrt{x}),b:=G(\sqrt{x})$, we see that
				\begin{align*}
			...
			Taking $x\to\infty$, we have
				...
				\end{align*}
			If $L(1,x)=0$, then $\varUpsilon(x^{2})=O(1)$ which is impossible since $\varUpsilon(x)\to\infty$. Therefore, we must have $L(1,x)\neq0$.
		\end{proof}
