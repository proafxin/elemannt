...
\begin{document}
	\chapter{Elementary Proofs of Legendre-Dirichlet Prime Number Theorem}\label{ch:legendre}
	...
	So, $L$-series is the \nameref{def:euler-product} when $f(n):=\chi(n)$. Since $\chi$ is completely multiplicative, using properties of Euler product, we immediately have the following.
		...
		\begin{proof}
			Since $\chi$ is completely multiplicative and $\varsigma$ is the summatory function on divisors, $\varsigma$ is also multiplicative. So we mainly need to look at
				\begin{align*}
					\varsigma(p^{e})
	...
	We omit the proof since it is straightforward at this stage. The stage is now set to finally prove Legendre's conjecture. Now we simply filter the sums related to $L(s, \chi)$ and $L'(s, \chi)$ to sieve out the primes of the form $kn+l$.
		...
		\end{note}
	\section{Second Proof by Selberg}\index{Selberg's proof of Dirichlet's theorem}
	...
	Here, we are not actually omitting the factor $2$. Rather we are simply considering that the contribution from $p^{u}q^{v}$ is $2\log{p}\log{q}$ whereas $q^{v}p^{u}$ is $0$ since both are actually same even though by order only which does not matter. So instead we can simply consider it as if both $p^{u}q^{v}$ and $q^{v}p^{u}$ contribute $\log{p}\log{q}$ which actually makes our calculation easier while keeping the total invariant. Again, we will treat $u=1$ and $u>1$ separately.
		...
		\end{align*}
	You can probably guess by now that the reason we treat them separately is because largest contribution towards the sum is made by the $p\leq x$ rather than $p^{2}\leq x,p^{3}\leq x,\ldots$ since they are much rarer. This is not a new idea. In fact, this was the original idea behind Tchebyscheff's $\vartheta(x)\sim\psi(x)$. The contributions of $\log{p}$ we get from $p^{2},p^{3},\ldots\leq x$ is negligible compared to what we get from $p\leq x$ alone when $x\to\infty$. Selberg makes a clever use of this idea here.
		\begin{align*}
	...
	Next, for when $u\geq2$ and $n=p^{u}q^{v}$,
		...
		\end{align*}
	Using $\log_{a}{b}\log_{b}{c}=\log_{a}{c}$,
		\begin{align*}
			\sum_{\substack{p^{u}q^{v}\leq x\\u\geq2\\p^{u}q^{v}\equiv l\pmod{k}}}\log{p}{\log{q}}
	...
	Same as before, we also calculate
		...
		\end{align*}
	For the second part of the right sum using partial summation formula,
		\begin{align*}
			\sum_{p\leq z}\dfrac{\log{p}}{p}\log\parenthesis{\frac{x}{p}}
				& = \log{x}\sum_{p\leq z}\dfrac{\log{p}}{p}-\sum_{p\leq z}\dfrac{\log^{2}{p}}{p}\\
			...
			\sum_{\sqrt[3]{x}<p\leq x}\dfrac{\log{p}}{p}\sum_{\substack{q\leq x/p\\pq\equiv l\pmod{k}}}\dfrac{\log{q}}{q}
				...
				& = \dfrac{2}{\varphi(k)}\sum_{\sqrt[3]{x}<p\leq x}\dfrac{\log{p}}{p}\log\parenthesis{\frac{x}{p}}\\
				& +\bigo{\log{x}\log\log{x}}\\
				& = \dfrac{2}{\varphi(k)}\parenthesis{\sum_{p\leq x}\dfrac{\log{p}}{p}\log\parenthesis{\frac{x}{p}}-\sum_{p\leq \sqrt[3]{x}}\dfrac{\log{p}}{p}\log\parenthesis{\frac{x}{p}}}\\
				& +\bigo{\log{x}\log\log{x}}\\
				& = \dfrac{2}{\varphi(k)}\parenthesis{\dfrac{1}{2}\log^{2}{x}-\dfrac{1}{3}\log^{2}{x}+\dfrac{1}{18}\log^{2}{x}+O(\log{x})}\\
				& +O(\log{x}\log{\log{x}})\\
	...
	For brevity, let
		\begin{align*}
			S(k,x)
				...
				& = \dfrac{\log^{2}{x}}{\varphi(k)}+O(\log{x})
		\end{align*}
	We don't need to consider $l$ here because the expression is free of $l$. This gives us
		\begin{align*}
			\sum_{\substack{p\leq x\\p\equiv l\pmod{k}}}\dfrac{\log^{2}{p}}{p}
	...
	Therefore,
		...
		\end{align*}
	Since $0\leq \log_{x}{\log{x}}<1$ for $x\geq e$, there is a real number $x_{0}(k)>0$ such that for all $x>x_{0}(k)$, $c\delta\log_{x}{\log{x}}$ is positive for any positive $c$. For such $x_{0}(k)$,
		\begin{align*}
			\dfrac{1}{9\varphi(k)}\log^{2}{x}+\bigo{\log_{x}{\log{x}}}
				& > \dfrac{1}{10\varphi(k)}\log^{2}{x}
		\end{align*}
	Following Selberg, if we set
		\begin{align*}
			Q_{l}(x)
	...
	we get
		\begin{align*}
			Q_{l}(x)
				& > \dfrac{1}{10\varphi(k)}-\dfrac{1}{9}\sum_{uv\equiv l\pmod{k}}\parenthesis{\dfrac{1}{\log{\sqrt[3]{x}}}\sum_{\substack{p\leq \sqrt[3]{x}\\p\equiv u\pmod{k}}}\dfrac{\log{p}}{p}}\parenthesis{\dfrac{1}{\log{\sqrt[3]{x}}}\sum_{\substack{q\leq \sqrt[3]{x}\\q\equiv v\pmod{k}}}\dfrac{\log{q}}{q}}\\
				& >\dfrac{1}{10\varphi(k)}-\dfrac{1}{9}\sum_{uv\equiv l\pmod{k}}Q_{u}(\sqrt[3]{x})Q_{v}(\sqrt[3]{x})
		\end{align*}
	Using partial summation formula and the notation $S$,
		\begin{align*}
			\sum_{\substack{p\leq x\\p\equiv l\pmod{k}}}\dfrac{\log^{3}{p}}{p}+\sum_{\substack{pq\leq x\\pq\equiv l\pmod{k}}}\dfrac{\log{p}\log{q}}{pq}\log{pq}
				& = S(k, x)\log{x}-\int_{1}^{x}\dfrac{S(k, t)}{t}dt\\
				& = \dfrac{\log^{3}{x}}{\varphi(k)}+O(\log^{2}{x})-\dfrac{1}{\varphi(k)}\int_{1}^{x}\dfrac{\log^{2}{x}}{t}dt-\int_{1}^{x}\dfrac{\log{t}}{t}dt\\
				& = \dfrac{2\log^{3}{x}}{3\varphi(k)}+\bigo{\log^{2}{x}}
		\end{align*}
	Here,
		\begin{align*}
			\sum_{\substack{pq\leq x\\pq\equiv l\pmod{k}}}\dfrac{\log{p}\log{q}}{pq}\log{pq}
				& = \sum_{\substack{pq\leq x\\pq\equiv l\pmod{k}}}\dfrac{\log{p}\log{q}}{pq}(\log{p}+\log{q})\\
				& = \sum_{\substack{pq\leq x\\pq\equiv l\pmod{k}}}\dfrac{\log{p}\log^{2}{q}}{pq}+\sum_{\substack{pq\leq x\\pq\equiv l\pmod{k}}}\dfrac{\log^{2}{p}\log{q}}{pq}\\
				& = 2\sum_{\substack{pq\leq x\\pq\equiv l\pmod{k}}}\dfrac{\log{p}\log{q}}{pq}\log^{2}{q}\\
				& = 2\sum_{p\leq x}\dfrac{\log{p}}{p}\sum_{\substack{q\leq x/p\\pq\equiv l\pmod{k}}}\dfrac{\log^{2}{q}}{q}
		\end{align*}
	Now, using the definition of $S(lp', x/p)$ where $pp'\equiv1\pmod{k}$ so $p'$ is the modular inverse of $p$,
		\begin{align*}
			\sum_{\substack{q\leq x/p\\pq\equiv l\pmod{k}}}\dfrac{\log^{2}{q}}{q}
				& = S(lp', x/p)-\sum_{\substack{pqr\leq x\\pqr\equiv l\pmod{k}}}\dfrac{\log{q}\log{r}}{qr}+O(\log{x})
		\end{align*}
	Moreover,
		\begin{align*}
			S(lp', x/p)
				& = \dfrac{\log^{2}(x/p)}{\varphi(k)}+\bigo{\log(x/p)}\\
			\sum_{p\leq x}\dfrac{\log{p}}{p}\sum_{\substack{q\leq x/p\\pq\equiv l\pmod{k}}}\dfrac{\log^{2}{q}}{q}
				& = \sum_{p\leq x}\dfrac{\log{p}}{p}\parenthesis{\dfrac{\log^{2}(x/p)}{\varphi(k)}+\bigo{\log(x/p)}}\\
				& = \dfrac{\log^{2}{x}}{\varphi(k)}\sum_{p\leq x}\dfrac{\log{p}}{p}-\dfrac{2\log{x}}{\varphi(k)}\sum_{p\leq x}\dfrac{\log^{2}{p}}{p}+\dfrac{1}{\varphi(k)}\sum_{p\leq x}\dfrac{\log^{3}{p}}{p}+O(\log^{2}{x})\\
				& = \dfrac{1}{3\varphi(k)}+O(1)+O(\log^{2}{x})
		\end{align*}
	Thus we have
		\begin{align*}
			\sum_{\substack{pq\leq x\\pq\equiv l\pmod{k}}}\dfrac{\log{p}\log{q}}{pq}\log{pq}
				& = \dfrac{2\log^{3}{x}}{3\varphi(k)}-2\sum_{\substack{pqr\leq x\\pqr\equiv l\pmod{k}}}\dfrac{\log{p}\log{q}\log{r}}{pqr}+O(\log^{2}{x})\\
			\sum_{\substack{p\leq x\\p\equiv l\pmod{k}}}\dfrac{\log^{3}{p}}{p}
				& = 2\sum_{\substack{pqr\leq x\\pqr\equiv l\pmod{k}}}\dfrac{\log{p}\log{q}\log{r}}{pqr}+\bigo{\log^{2}{x}}
		\end{align*}
	Going back to the definition of $Q_{l}(x)$,
		\begin{align*}
			Q_{l}(x)
				& \geq \dfrac{2}{\log^{3}{x}}\sum_{\substack{p,q,r\leq\sqrt[3]{x}\\pqr\equiv l\pmod{k}}}\dfrac{\log{p}\log{q}\log{r}}{pqr}+\bigo{\dfrac{1}{\log{x}}}\\
				& = \dfrac{2}{27}\sum_{\substack{uvw\equiv l\pmod{k}}}Q_{u}(\sqrt[3]{x})Q_{v}(\sqrt[3]{x})Q_{w}(\sqrt[3]{x})+\bigo{\dfrac{1}{\log{x}}}
		\end{align*}
